...
const text = (vi: string, en: string, zh: string): Text => ({
  vi,
  en,
  "zh-TW": zh,
});
export default function OrderingExperience({ qrToken }: { qrToken: string }) {
  ...
  const categories = useMemo(
    () => [
      ...new Map(
        menu.map((item) => [item.category.vi, item.category]),
      ).values(),
    ],
    [menu],
  );
  const visible =
    category === "all"
      ? menu
      : menu.filter((item) => item.category.vi === category);
  const total = cart.reduce(
    (sum, line) =>
      sum +
      line.quantity *
        (line.product.price +
          line.addons.reduce((extra, addon) => extra + addon.priceExtra, 0)),
    0,
  );
  useEffect(() => {
    let active = true;
  ...
  const toggleAddon = (addon: Addon) =>
    !addon.unavailable &&
    setAddons((old) =>
      old.some((value) => value.id === addon.id)
        ? old.filter((value) => value.id !== addon.id)
        : [...old, addon],
    );
  const add = () => {
    if (!selected) return;
  ...
  const sendOrder = async () => {
    ...
    try {
      const created = await fetch(
        `/api/public/qr/${encodeURIComponent(qrToken)}/carts`,
        {
          method: "POST",
        },
      );
      ...
      const confirmed = await fetch(`/api/public/carts/${cartToken}/confirm`, {
        method: "POST",
      });
                ...
                <div className="addon-list">
                  {selected.addons.map((item) => (
                    <button
                      ...
                      className={
                        addons.some((value) => value.id === item.id)
                          ? "selected"
                          : ""
                      }
            ...
            <div className="sheet-footer">
              <strong>
                {price(
                  quantity *
                    (selected.price +
                      addons.reduce((sum, item) => sum + item.priceExtra, 0)),
                )}
              </strong>
