...
export default function LiveQrOrder({ qrToken }: { qrToken: string }) {
    ...
    try {
      ...
      if (savedCartToken) {
        ...
        if (saved.ok && (await saved.clone().json()).data.status === 'draft') {
          ...
          setCart(savedData.lines.map((line: Omit<CartLine, 'key'>) => ({ ...line, key: crypto.randomUUID() })))
        } else {
  ...
  const openItem = (item: MenuItem) => { setSelected(item); setQuantity(1); setVariant(item.variants[0]?.id || ''); setNoteOptions([]); setAddonIds([]); setNote('') }
  ...
  const addToCart = () => {
    if (!selected) return
    setCart((old) => [...old, { key: crypto.randomUUID(), itemId: selected.id, quantity, variant: variant || undefined, noteOptions, addonIds, note: note.trim() || undefined }])
    setSelected(null)
  }
  ...
  if (loading || failed) return <main className="page"><section className="card" aria-live="polite"><div className="brand">{copy.brand}</div><h1>{failed ? copy.menuUnavailable : copy.qrMenuLoading}</h1><p>{failed ? copy.menuUnavailableDescription : copy.qrMenuDescription}</p></section></main>
