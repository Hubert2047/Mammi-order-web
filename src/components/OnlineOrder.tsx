...
type MenuItem = { id: string; category: { id: string; names: Text }; names: Text; description: Text; price: number; displayPrice?: number; variants: Choice[]; noteOptions: Choice[]; addons: Addon[] }
...
export default function OnlineOrder() {
  ...
  const linePrice = (line: CartLine) => { const item = items.find((candidate) => candidate.id === line.itemId); return (item?.displayPrice ?? item?.price ?? 0) + (item?.addons.filter((addon) => line.addonIds.includes(addon.id)).reduce((sum, addon) => sum + (addon.displayPrice ?? addon.priceExtra), 0) || 0) }
  const catalogTotal = useMemo(() => cart.reduce((sum, line) => sum + line.quantity * linePrice(line), 0), [cart, items])
  ...
  useEffect(() => {
    ...
    for (const event of ['catalog.item.updated', 'catalog.store-item.price.updated', 'catalog.store-item.availability.updated', 'catalog.store-addon.updated', 'catalog.store-addon.availability.updated', 'catalog.promotion.updated', 'catalog.changed']) socket.on(event, refresh)
  ...
  useEffect(() => {
    ...
    if (cached && cached.key === quoteKey && cached.expiresAt > Date.now()) { setPromotionTotal(cached.total); return }
    const timer = window.setTimeout(() => {
      void fetch(`${base}/api/public/carts/${cartToken}/preview`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lines }) })
        .then((response) => response.ok ? response.json() : null)
        .then((payload) => {
          const total = payload?.data?.total
          const expiresAt = Date.parse(payload?.data?.expiresAt || '')
          if (typeof total !== 'number' || !Number.isFinite(expiresAt)) { setPromotionTotal(null); return }
  ...
  const confirm = async () => {
    ...
    try {
      const lines = cart.map(({ key, ...line }) => line)
      const synced = await fetch(`${base}/api/public/carts/${cartToken}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lines, type }) })
      ...
      if (!response.ok) {
        ...
        if (payload?.code === 'ONLINE_ORDER_RATE_LIMITED') { setOrderRateLimited(true); setFailed(true); return }
        throw new Error()
      }
  ...
  if (loading || failed) return <main className="page"><section className="card" aria-live="polite"><img className="error-logo" src="/logo.png" alt="" width="96" height="96" /><h1>{failed ? copy.menuUnavailable : copy.qrMenuLoading}</h1><p>{orderRateLimited ? copy.onlineOrderRateLimited : failed ? copy.menuUnavailableDescription : copy.qrMenuDescription}</p></section></main>
